...
class cfgInterface(Action):
    def run(self, controllerIP, controllerUser, controllerPW, deviceName, intfType, intfNum, address):
        ...
        url = url_base + deviceName + "/yang-ext:mount/vyatta-interfaces-v1:interfaces/"

        # Preapring the call's body (rely on Jinja2 template to build the body)
        #######################################################################
        f=open("/opt/stackstorm/packs/odl_vyatta/actions/templates/template_interface.j2")
        s=f.read()
        template=Template(s)
        body = template.render(intfType=intfType, intfNum=intfNum, address=address)
        print("\n")
        print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
