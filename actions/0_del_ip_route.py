...
class delIpRoute(Action):
    def run(self, controllerIP, controllerUser, controllerPW, deviceName, subnet, nextHop):
        ...
        url = url_base + deviceName + "/yang-ext:mount/vyatta-protocols-v1:protocols/vyatta-protocols-static-v1:static/"

        # Preapring the call's body (rely on Jinja2 template to build the body)
        #######################################################################
        f=open("/opt/stackstorm/packs/odl_vyatta/actions/templates/template_ip_route.j2")
        s=f.read()
        template=Template(s)
        body = template.render(subnet=subnet, nextHop=nextHop)
        print("\n")
        print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
        ...
        with warnings.catch_warnings():
            ...
            cmd_response = requests.delete(url, auth=(controllerUser, controllerPW), headers=headers, data=body, verify=False)
