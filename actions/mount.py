...
class mount(Action):
    def run(self, controllerIP, controllerUser, controllerPW, mountName, deviceIP, deviceUser, devicePW, deviceSshPort):
        ...
        #######################################################################
        f=open("/opt/stackstorm/packs/odl_vyatta/actions/templates/template_mount.j2")
        s=f.read()
        template=Template(s)
        body = template.render(mountName=mountName, deviceIP=deviceIP, deviceUser=deviceUser,devicePW=devicePW, deviceSshPort=deviceSshPort)
        print("\n")
        print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
